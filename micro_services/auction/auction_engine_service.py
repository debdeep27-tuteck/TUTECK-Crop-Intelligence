...
@app.route("/auctions/<auction_id>", methods=["PATCH"])
@require_api_key
def update_auction(auction_id):
    body = request.get_json(force=True) or {}
    engine = get_engine()
    row = engine.conn.execute("SELECT * FROM auctions WHERE id=?", (auction_id,)).fetchone()
    if row is None:
        return jsonify({"error": "not found"}), 404

    allowed = {
        "itemLabel", "category", "metadata", "targetQuantity",
        "remainingQuantity", "basePrice", "ownerPrice",
        "counterGap", "auctionType", "durationMinutes",
        "extensionMinutes", "startsAt", "endsAt", "status",
    }
    fields = []
    params = []
    ...
    if "metadata" in body:
        ...
        params.append(str(body["metadata"]).strip() or None)
    ...
    if not fields:
        return jsonify(engine.get_auction(auction_id))

    params.append(auction_id)
    engine.conn.execute(f"UPDATE auctions SET {', '.join(fields)} WHERE id=?", tuple(params))
    engine.conn.commit()
    return jsonify(engine.get_auction(auction_id))


@app.route("/auctions/<auction_id>", methods=["DELETE"])
@require_api_key
def delete_auction(auction_id):
    engine = get_engine()
    row = engine.conn.execute("SELECT id FROM auctions WHERE id=?", (auction_id,)).fetchone()
    if row is None:
        return jsonify({"error": "not found"}), 404

    engine.conn.execute("DELETE FROM bids WHERE auction_id=?", (auction_id,))
    engine.conn.execute("DELETE FROM invitations WHERE auction_id=?", (auction_id,))
    engine.conn.execute("DELETE FROM auctions WHERE id=?", (auction_id,))
    engine.conn.commit()
    return jsonify({"success": True, "deletedId": auction_id})
