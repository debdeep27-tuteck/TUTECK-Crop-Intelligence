...
def cached_route(seconds=30, key_prefix=None):
    ...
    def decorator(fn):
        ...
        @wraps(fn)
        def wrapper(*args, **kwargs):
            # Only GET is safe to cache by path+query alone — a route
            # registered for GET and POST/PUT/etc. would otherwise risk
            # serving a cached GET body in place of a real POST
            # computation, since the cache key doesn't cover the method
            # or request body. Non-GET requests always bypass the cache.
