...
def load_and_join(data_path=DATA_PATH):
    ...
    if "Season" in df.columns:
        df["Season"] = df["Season"].astype(str).str.strip()
    # NOTE: this Excel file may already contain weather_* columns baked in
    # from a previous run of this same pipeline. If we don't drop them here,
    # the merge below (which also produces weather_* columns) collides and
    # pandas silently renames both sides to weather_temp_mean_x/_y, which is
    # exactly what caused the "KeyError: weather_temp_mean" crash.
    df = df.drop(columns=["Production (Tonnes/Bales)"] + WEATHER_FEATURES, errors="ignore")
